...
# ================== DB SAVING FUNCTION ==================
def save_to_database(gdf, table_name, project_id, name):
    ...
    try:
        ...
        # 2. Convert Geometry to WKT (Text)
        ...
        # 4. SELECT ONLY COLUMNS THAT EXIST IN DATABASE
        allowed_cols = ['project_id', 'project_name', 'geometry']
        
        if 'block_id' in df_db.columns:
            allowed_cols.append('block_id')
        if 'zone_id' in df_db.columns:
            allowed_cols.append('zone_id')
        if 'cluster_id' in df_db.columns:
            allowed_cols.append('cluster_id')

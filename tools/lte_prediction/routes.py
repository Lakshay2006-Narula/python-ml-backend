...
def run_prediction():
    ...
    if operators is not None:
        ...
        if not isinstance(operators, list):
            return jsonify({"error": "'operators' must be a string or list of strings"}), 400
        operators = [str(op).strip() for op in operators if str(op).strip()]
        if not operators:
            operators = None
    ...
    cfg = {
        ...
        # Optional operator-wise filtering/splitting
        "operators":         operators,
        "split_by_operator": to_bool(data.get("split_by_operator"), False),
...
def job_result(job_id):
    ...
    has_operator_dimension = bool(job.get("has_operator_dimension", False))
    refined_columns = [
        "lat", "lon", "site_id",
        "pred_rsrp_top2_avg", "pred_rsrp_top3_avg", "measured_dt_rsrp"
    ]
    if has_operator_dimension:
        refined_columns.insert(0, "operator")
    ...
    return jsonify({
        ...
        "pipeline": {
            ...
            "operators": {
                "requested":         job.get("operators_requested", []),
                "used":              job.get("operators_used", []),
                "split_by_operator": has_operator_dimension,
                "pixel_counts":      job.get("operator_pixel_counts", {}),
            },
        },
    }), 200

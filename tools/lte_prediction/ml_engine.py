...
def _run_prediction_pass(site_df: pd.DataFrame, params: dict,
                          ...
                          label: str,
                          split_by_operator: bool = False) -> pd.DataFrame:
    """
    ...
    has_operator_dimension = split_by_operator and "operator" in site_df.columns
    ...
    for site in site_df[site_col].unique():
        ...
        if has_operator_dimension:
            pts_df["operator"] = site_rows["operator"].iloc[0]
    ...
    group_cols = ["lat", "lon"]
    if has_operator_dimension and "operator" in final_output.columns:
        group_cols = ["operator"] + group_cols

    final_output = final_output.groupby(group_cols, as_index=False).agg(
...
def run_two_pass_prediction(
    ...
    group_cols = ["lat", "lon"]
    if has_operator_dimension and "operator" in pass1_result.columns:
        group_cols = ["operator"] + group_cols

    pass1_result = pass1_result.groupby(group_cols, as_index=False).agg(
        pred_rsrp=("pred_rsrp", "mean"),
        pred_rsrq=("pred_rsrq", "mean"),
        pred_sinr=("pred_sinr", "mean"),
        site_id  =("site_id",   "first"),
    ...
    synth_cols = ["lat", "lon", "site_id", top2_col]
    if has_operator_dimension and "operator" in enriched_result.columns:
        synth_cols.insert(2, "operator")

    synthetic_dt_full = (
        enriched_result[synth_cols]
    ...
    for site in site_df[site_col].unique():
        ...
        if len(synthetic_dt) >= 10:
            ...
            site_operator = (
                site_rows["operator"].iloc[0]
                if has_operator_dimension and "operator" in site_rows.columns
                else None
            )

            if site_operator is not None and "operator" in site_synthetic.columns:
                site_synthetic = site_synthetic[
                    site_synthetic["operator"] == site_operator
                ].copy()

            if len(site_synthetic) < 10:
                if site_operator is not None and "operator" in synthetic_dt.columns:
                    site_synthetic = synthetic_dt[
                        synthetic_dt["operator"] == site_operator
                    ].copy()
                    if len(site_synthetic) < 10:
    ...
    group_cols = ["lat", "lon"]
    if has_operator_dimension and "operator" in pass2_result.columns:
        group_cols = ["operator"] + group_cols

    pass2_result = pass2_result.groupby(group_cols, as_index=False).agg(
        pred_rsrp=("pred_rsrp", "mean"),
        pred_rsrq=("pred_rsrq", "mean"),
        pred_sinr=("pred_sinr", "mean"),
        site_id  =("site_id",   "first"),
    ...
    operators = []
    operator_pixel_counts = {}
    if has_operator_dimension and "operator" in pass2_result.columns:
        operators = sorted(pass2_result["operator"].dropna().astype(str).unique().tolist())
        operator_pixel_counts = {
            str(operator): int(count)
            for operator, count in pass2_result.groupby("operator").size().items()
        }
    ...
    return {
        ...
        "meta": {
            ...
            "has_operator_dimension": has_operator_dimension,
            "operators":              operators,
            "operator_pixel_counts":  operator_pixel_counts,
        },
    }
